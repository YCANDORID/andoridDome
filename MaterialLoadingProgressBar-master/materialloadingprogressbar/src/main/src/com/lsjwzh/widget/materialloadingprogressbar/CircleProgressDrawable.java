...
public class CircleProgressDrawable extends Drawable implements Animatable {
	...
	public void setSizeParameters(double progressCircleWidth,
	 ...
	 * 
	 * @param size
	 *            One of {@link CircleProgressDrawable.LARGE} or
	 *            {@link CircleProgressDrawable.DEFAULT}
	 */
	public void updateSizes(@ProgressDrawableSize int size) {
		...
		if (size == LARGE) {
			setSizeParameters(CIRCLE_DIAMETER_LARGE * screenDensity,
					CIRCLE_DIAMETER_LARGE * screenDensity, CENTER_RADIUS_LARGE
							* screenDensity,
					STROKE_WIDTH_LARGE * screenDensity, ARROW_WIDTH_LARGE
							* screenDensity, ARROW_HEIGHT_LARGE * screenDensity);
		} else {
			setSizeParameters(CIRCLE_DIAMETER * screenDensity, CIRCLE_DIAMETER
					* screenDensity, CENTER_RADIUS * screenDensity,
					STROKE_WIDTH * screenDensity, ARROW_WIDTH * screenDensity,
	...
	private static class Ring {
		...
		private void drawTriangle(Canvas c, float startAngle, float sweepAngle,
			...
			if (mShowArrow) {
				...
				mArrow.offset(x * 1.25f, y);// -((mArrowWidth) * mArrowScale /
											// 2)//Change the arrow place
				mArrow.close();
				// draw a triangle
				mArrowPaint.setColor(mColors[mColorIndex]);
				// when sweepAngle < 0 adjust the position of the arrow
				c.rotate(startAngle + (sweepAngle < 0 ? 0 : sweepAngle)
						- ARROW_OFFSET_ANGLE, bounds.exactCenterX(),
						bounds.exactCenterY());
		...
		public void setInsets(int width, int height) {
			...
			if (mRingCenterRadius <= 0 || minEdge < 0) {
				insets = (float) Math.ceil(mStrokeWidth / 3.5f);
			} else {
				insets = (float) (minEdge / 3.5f - mRingCenterRadius);
			}
			mStrokeInset = insets;
		}
